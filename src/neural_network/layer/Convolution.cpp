...
{
    std::vector<float> errorByFilter(this->numberOfFilters, 0.0F);
    for (size_t e = 0; e < inputErrors.size();)
    {
        for (int f = 0; f < this->numberOfFilters; ++f, ++e)
        {
            errorByFilter[f] += inputErrors[e];
        }
    }
    for (int f = 0; f < this->numberOfFilters; ++f)
    {
        this->neurons[f].back(errorByFilter[f]);
