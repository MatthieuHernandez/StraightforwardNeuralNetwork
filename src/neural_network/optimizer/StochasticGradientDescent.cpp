...
void StochasticGradientDescent::updateWeights(SimpleNeuron& neuron) const
{
    ...
    const auto lastInputs = neuron.lastInputs.getAverage();
    auto& deltaWeights = neuron.deltaWeights;
    auto& weights = neuron.weights;
    const auto lr_error = this->learningRate * neuron.lastError;
    // #pragma omp simd
    for (w = 0; w < numberOfInputs; ++w)
    {
        deltaWeights[w] = lr_error * lastInputs[w] + m * deltaWeights[w];
        weights[w] += deltaWeights[w];
    }
    deltaWeights[w] = lr_error * neuron.bias + m * deltaWeights[w];
    weights[w] += deltaWeights[w];
}
...
void StochasticGradientDescent::updateWeights(RecurrentNeuron& neuron) const
{
    ...
    weights[w] += deltaWeights[w];
    neuron.recurrentError = neuron.lastError;  // + neuron.recurrentError *
