...
TEST_F(Cifar10Test, DISABLED_trainBestNeuralNetwork)
{
    StraightforwardNeuralNetwork neuralNetwork({
        Input(32, 32, 3),
        Convolution(3,4, activation::ReLU),
        FullyConnected(250, activation::sigmoid),
        FullyConnected(10)
    },
    ...
    neuralNetwork.train(*data, 0.5417_acc || 25_ep);
...
TEST_F(Cifar10Test, EvaluateBestNeuralNetwork)
{
    ...
    auto accuracy = neuralNetwork.getGlobalClusteringRate();
    ASSERT_EQ(numberOfParameters, 754354);
    ASSERT_FLOAT_EQ(accuracy, 0.5592f);
}