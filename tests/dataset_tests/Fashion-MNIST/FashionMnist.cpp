...
FashionMnist::FashionMnist(const std::string& folderPath) { this->loadData(folderPath); }
...
auto FashionMnist::readImages(std::string filePath, int size) -> snn::vector2D<float>
{
    std::ifstream file;
...
auto FashionMnist::readLabels(std::string filePath, int size) -> snn::vector2D<float>
{
    std::ifstream file;
    file.open(filePath, std::ios::in | std::ios::binary);  // NOLINT(hicpp-signed-bitwise)
