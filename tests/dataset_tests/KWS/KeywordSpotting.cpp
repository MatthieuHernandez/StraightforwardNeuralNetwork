...
KeywordSpotting::KeywordSpotting(const std::string& folderPath, int sizeOfOneData)
    : sizeOfOneData(sizeOfOneData)
{
    this->loadData(folderPath);
}

void KeywordSpotting::loadData(const std::string& folderPath)
{
    std::array<std::vector<std::string>, 2> filePaths;
    filePaths[0] = snn::tools::getFilePaths(folderPath + "/training", ".wav");
    filePaths[1] = snn::tools::getFilePaths(folderPath + "/testing", ".wav");
    ...
    for (int i = 0; i < numberOfSet; ++i)
    {
        for (const auto& filePath : filePaths.at(i))
        {
            ...
            if (filePath.find("helloworld") != std::string::npos)
            {
                expected = {1, 0, 0};
            }
            else if (filePath.find("noise") != std::string::npos)
            {
                expected = {0, 1, 0};
            }
            else if (filePath.find("unknown") != std::string::npos)
            {
                expected = {0, 0, 1};
            }
            else
