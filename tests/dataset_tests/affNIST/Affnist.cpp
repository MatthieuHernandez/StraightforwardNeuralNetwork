...
Affnist::Affnist(const std::string& folderPath) { this->loadData(folderPath); }

void Affnist::loadData(const std::string& folderPath)
{
...
auto Affnist::readImages(std::string filePath, int size) -> snn::vector2D<float>
{
    std::ifstream file;
    file.open(filePath, std::ios::in | std::ios::binary);  // NOLINT(hicpp-signed-bitwise)
    snn::vector2D<float> images;
    images.reserve(size);
    constexpr int sizeOfData = 40 * 40;
...
auto Affnist::readLabels(std::string filePath, int size) -> snn::vector2D<float>
{
    std::ifstream file;
    file.open(filePath, std::ios::in | std::ios::binary);  // NOLINT(hicpp-signed-bitwise)
